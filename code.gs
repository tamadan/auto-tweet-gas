    ...
    times.forEach(time_candidates => {
      if(time_candidates.includes('~')) {
        time_candidates.split('~').forEach(time_candidate => {
          ...
          time_candidates += `${wait_date.getHours().toString().padStart(2, '0')}:${wait_date.getMinutes().toString().padStart(2, '0')}~`
        })
        time_candidates = time_candidates.replace(/(.*)\~/, "$1,")
      } else {
          const wait_date = new Date(`${new Date().toLocaleDateString()} ${String(time_candidates)}`)
          time_candidates += `${wait_date.getHours().toString().padStart(2, '0')}:${wait_date.getMinutes().toString().padStart(2, '0')},`
      }
    })
    time_candidates.slice(0, -1);
  }
...
function sendHttpPost(value, stage){
   ...
   const { schedule_cron, time_candidates } = convertCronTime(value[DB_DATA.time], value[DB_DATA.week])
   const payload =
   {
     "id": value[DB_DATA.id],
